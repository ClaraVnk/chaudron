...
def undocumented(name: str = "Farine de blé") -> ProductFacts:
    ...
    The same holds for ``avoided_ingredients_risk`` and the whole avoidable
    vocabulary: the two columns are generated by the same shape of CASE, and a
    fixture that reproduced one and not the other would test half a screen.
    """
    return facts(
        name,
        state=AllergenDataState.UNKNOWN,
        risk=frozenset(Allergen),
