...
class OpenFoodFactsCatalog:
    ...
    async def lookup(self, gtin: str) -> CatalogRecord | None:
        ...
        return _to_record(gtin, product)
...
def _first_string(product: dict[str, Any], *keys: str) -> str | None:
    for key in keys:
...
def _to_record(gtin: str, product: dict[str, Any]) -> CatalogRecord:
    ...
    without rescanning the article.
    """
    ...
    return CatalogRecord(
        ...
        name=_first_string(product, "product_name_fr", "product_name", "generic_name_fr")
        or display_gtin(gtin),
        brand=_first_string(product, "brands"),
        image_url=_first_string(product, "image_front_url", "image_url"),
