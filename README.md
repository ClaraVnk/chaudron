...
| 📦 **Stock tracking** | ✅ built | What you own, where it's stored, and when it expires — per household, not per person. Use-by and best-before are distinct: conflating them means either anxious alerts on dry pasta or silence on minced beef. Items can be corrected and removed after the fact, including **the location they were filed in** — a wrong one is not cosmetic, since the freezer suspends the expiry date. Dates are **typed** rather than picked: `12/08/2026`, `120826`, or the calendar if you prefer it. A bare `12/08` is refused rather than resolved against "soon", because guessing the year on a use-by is how food is thrown away a year early. |
...
| 📷 **Barcode scanning** | ✅ built | Decoded **in the browser** — the server only ever sees thirteen characters, never a video stream. Products resolve through [Open Food Facts](https://world.openfoodfacts.org/). Putting away a shopping trip means twenty items in a row, so the camera **stays on between them** — it is released when you leave, or after ninety seconds, because a camera left running in a kitchen is a camera left running. |
| 🍳 **Recipe suggestions** | ✅ built | Generated from the stock actually on hand. Whether an ingredient is in stock is **recomputed against your inventory**, never taken from the model's word for it. Ask for **baking** rather than a meal and the request goes as a structured value, not as a wish in a free-text box that argues with the instruction above it — and it asks for a refusal rather than a cake scaled down until it stops working, because a stew tolerates roughly enough onion and a cake does not tolerate roughly enough flour. |
...
| 🥝 **Avoided ingredients** | ✅ built | Kiwi, strawberry, celeriac — the allergies and dislikes that are not among the regulated fourteen. Matched against the ingredient list Open Food Facts publishes, and kept **visibly apart** from the allergen list, because only one of the two rests on a declaration a manufacturer was obliged to make. How an *unreadable* list is read is your choice, per person, and the measurement is why: on 1 263 864 French products only **13.2%** carry a list this parser can read, and 72% of packaged products carry none at all. Treating unknown as unsafe — right for the regulated allergens — therefore withholds **86.8% of the catalogue**, which is not a cautious filter but a switch that turns the inventory off. So the default withholds only what a list positively *names* and reports the rest as undocumented; the strict reading is one tick away, with that cost stated on the screen. A table seating a diagnosis and a dislike is catered for the diagnosis. |
